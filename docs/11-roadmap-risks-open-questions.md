...
| Hot headroom unused by PAYG in some regions | Margin erosion | Shift to warm spares where PAYG demand is thin. The Multi-region surcharge (0.2× base) pays for reserved failover headroom |
...
|---|----------|----------|--------------------|
| 1 | How are burst and spillover priced? | **Burst credit is free** within the configured cap. **Spillover is billed as regular PAYG traffic**, at the regular PAYG list price per model (`[[payg_prices]]` mirrors the PAYG price list). | [04 §4](04-request-lifecycle-and-admission.md#4-boundary-policies) |
...
| 9 | How is the Multi-region SKU priced? (It reserves failover headroom in a paired region, ADR-014.) | A **surcharge of 0.2× the base CU price** per CU on top of the tier price (Standard 1.2×, Interactive 1.45×, Agentic 1.7×). It adds to the strict-dedicated surcharge (Agentic strict-dedicated Multi-region: 2.0×). | [02 §3](02-capacity-unit-and-cost-model.md#3-capacity-unit-cu-and-slo-tiers), [07 §2](07-multi-region.md#2-reservation-geography) |

Questions 1 and 9 were settled on 2026-09-24. No pricing questions are open.
