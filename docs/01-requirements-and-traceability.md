...
| P10 | Large weights make cold start slow | R10: Replacement replica serving in under 2 minutes (warm tier) | NVMe weight cache, P2P distribution, streaming loader; warm spares loaded for failover demand | [06 §4](06-capacity-planning-and-reliability.md#4-fast-model-loading), [08 §2](08-kubernetes-and-dynamo-integration.md#2-custom-resources) |
| P11 | Admission must estimate cost before output length is known | R11: Accurate estimate plus settlement | Exact prefill from the model's tokenizer, cached per message, with a learned ratio for new text beyond the inline budget; expected cache hits from the gateway's prefix index at a learned hit rate; learned decode estimate; debt bucket | [04 §3](04-request-lifecycle-and-admission.md#3-cost-estimation), [ADR-002](adr/ADR-002-debt-based-wu-bucket.md) |
...
| P16 | Isolation strategies: dedicated, shared, or hybrid | R16: Tiered pool model | Shared-provisioned, dedicated with backfill, PAYG, spare; backfill capped per floor worker; interference suite | [05 §6](05-isolation-and-scheduling.md#6-pool-tiers-hybrid-isolation), [05 §7](05-isolation-and-scheduling.md#7-validation-interference-test-suite) |
...
|----|-------------|--------|
| N1 | Gateway admission overhead (tokenise + estimate + admit) | p99 < 2 ms when a request's new (uncached) content fits the inline tokenisation budget; repeated context is a cache lookup ([ADR-028](adr/ADR-028-input-token-counting.md)) |
