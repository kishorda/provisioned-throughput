...
| P9 | Reliability: GPU errors, node loss, drains, upgrades, hardware refresh | R9: N+k per failure domain; warm spares; fast load; capacity-aware drain | Planner headroom; spare tiers; drain controller | [06](06-capacity-planning-and-reliability.md) |
...
| P12 | Boundary behaviour: reject, queue, spillover, burst | R12: Customer-configurable boundary policy | `BoundaryPolicy` per deployment | [04 §4](04-request-lifecycle-and-admission.md#4-boundary-policies) |
...
| P15 | Noisy neighbour: one tenant's burst raises everyone's TTFT | R15: Cost-weighted fair queuing | Router WFQ by WU; pull-based dispatch | [05 §3](05-isolation-and-scheduling.md#3-level-2--router) |
...
| P17 | Provisioned traffic must preempt on-demand traffic | R17: Strict priority classes | `provisioned > burst > spillover/PAYG` | [05 §2](05-isolation-and-scheduling.md#2-priority-classes) |
